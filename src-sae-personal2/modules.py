...
# 串联：attent串联crossatten
class LateFusionEncoder_c(nn.Module):
    def __init__(self, args, latent_init_tensor, trainable_latent=False, fusion_type='add'):
        super(LateFusionEncoder_c, self).__init__()
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        # 2. Cross-attention branch
        cross_branch = self_branch
        attention_list = []
        for layer_module in self.cross_layers:
            cross_branch, attention_probs = layer_module(cross_branch, memory_tensor)
            attention_list.append(attention_probs)

        fused_output = cross_branch
        # fused_output = self.LayerNorm(cross_branch)
...
class LateFusionEncoder_gating(nn.Module):
    ...
    '''
    def forward(self, hidden_states, attention_mask, topk_ids, output_all_encoded_layers=True):
        '''
        hidden_states: (b,l,64)
        topk_ids: (b,5)
        self.memory_tensor: (1000,768)
        '''
        memory_tensor = self.memory_tensor  # 固定或可学习memory: (1000, 768)
        # 提取 intent 向量
        memory_tensor = memory_tensor[topk_ids]  # 结果 shape: (b, 5, 768)
        #print("1:", memory_tensor.shape)
        memory_tensor = self.memory_projector(memory_tensor) 
        #print("2:", memory_tensor.shape)
        #pirnt()
        ...
        attention_list = []
        for layer_module in self.cross_layers:
            cross_branch, attention_probs = layer_module(cross_branch, memory_tensor) # (b,5,64)
            attention_list.append(attention_probs)
...
class LateFusionEncoder3(nn.Module):
    def __init__(self, args, latent_init_tensor, trainable_latent=False, fusion_type='add'):
        super(LateFusionEncoder3, self).__init__()
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        topk_vals, topk_idx = torch.topk(intent_scores_soft.mean(dim=1), k=1, dim=-1)  # (b, 1)
        topk_intents = memory_tensor[topk_idx.squeeze()]  # (b, d)
        loss_align = 1 - F.cosine_similarity(user_intent_repr, topk_intents, dim=-1).mean()
        # # 或：
        # loss_align = F.mse_loss(user_intent_repr, topk_intents)
        

        return user_intent_repr, loss_align
