    ...
    def iteration(self, epoch, dataloader, cluster_dataloader=None, full_sort=True, train=True):
        ...
        if train:
            ...
            for i, (rec_batch) in rec_cf_data_iter:
                ...
                rec_batch = tuple(t.to(self.device) for t in rec_batch)
                topk_ids, user_ids, subsequence_1, target_pos_1, subsequence_2, answers = rec_batch
                #print("用户的topk_ids: list:", len(topk_ids), topk_ids)
                #print("用户的topk_ids:", topk_ids, user_ids, user_ids.shape, type(topk_ids) )
                #print("用户的topk_ids:", type(topk_ids), topk_ids.shape ) # 用户的topk_ids: <class 'torch.Tensor'> torch.Size([256, 10])
                
                # topk_ids = torch.unique(topk_ids)
                # print(type(topk_ids), topk_ids.shape)
                # pirnt()
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    topk_ids, user_ids, input_ids, target_pos, answers = batch
                    # topk_ids = torch.unique(topk_ids)
                    # print("valid:", type(topk_ids), topk_ids.shape)

                    recommend_output, memory_tensor, query_repr, attention_scores_mean = self.model(input_ids, topk_ids) # [BxLxH]
...
class ICSRecTrainer3(Trainer):
    def __init__(self, model, train_dataloader,cluster_dataloader,eval_dataloader, test_dataloader, args):
        super(ICSRecTrainer3, self).__init__(
            model, train_dataloader, cluster_dataloader,eval_dataloader, test_dataloader, args
        )
    ...
    def iteration(self, epoch, dataloader, cluster_dataloader=None, full_sort=True, train=True):
        ...
        if train:
            ...
            for i, (rec_batch) in rec_cf_data_iter:
                ...
                # ---------- prediction task -----------------------#
                user_intent_repr, loss_align = self.model(subsequence_1)
                logits = self.predict_full(user_intent_repr)  #  [Bx|I|]
                rec_loss = nn.CrossEntropyLoss()(logits, target_pos_1[:, -1])
                latent_loss = 0
                ...
                rec_avg_loss += rec_loss.item()
                latent_avg_loss += latent_loss
                align_avg_loss += align_loss.item()
                attention_avg_loss += attention_loss
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    user_intent_repr, loss_align = self.model(input_ids) # [BxLxH]
                    recommend_output = user_intent_repr # [BxH]
                    # recommendation results
                    rating_pred = self.predict_full(recommend_output)
                    rating_pred = rating_pred.cpu().data.numpy().copy()
                    batch_user_index = user_ids.cpu().numpy()
                ...
                for i, batch in rec_data_iter:
                    batch = tuple(t.to(self.device) for t in batch)
                    user_ids, input_ids, target_pos, target_neg, answers, sample_negs = batch
                    user_intent_repr,_ = self.model.finetune(input_ids)
                    test_neg_items = torch.cat((answers, sample_negs), -1)
                    recommend_output = user_intent_repr
