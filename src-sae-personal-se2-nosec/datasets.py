...
'''
RecWithContrastiveLearningDataset(Dataset)改动：适配user id，每条记录需要标明属于哪个用户
'''
class Generate_tag():
    ...
    def generate(self):
        data_f=self.path+"/"+self.data_name+".txt" # 读取的都是分割后的数据
        train_dic={}
        valid_dic={}
        test_dic={}
        with open(data_f,"r") as fr:
            ...
            for d_ in data:
                ...
                train_temp=list(map(int, items[:-3])) # 训练的用户序列
                valid_temp=list(map(int, items[:-2])) # 验证的用户序列
                test_temp=list(map(int,items[:-1])) # 测试的用户序列
                if tag_train not in train_dic:
                    train_dic.setdefault(tag_train,[]) # 加上某个item为tag
        ...
        total_dic={"train":train_dic,"valid":valid_dic,"test":test_dic} # 按照item分类，以某个item为target的一些用户序列在一个list中
...
class RecWithContrastiveLearningDataset(Dataset):
    def __init__(self, args, user_seq, user_id, tfidf_list, test_neg_items=None, data_type="train", similarity_model_type="offline"):
        self.args = args
        self.user_seq = user_seq # list嵌套list，每一条是一个用户序列
        self.test_neg_items = test_neg_items
        self.data_type = data_type
        self.max_len = args.max_seq_length
        self.user_id = user_id # list
        self.tfidf_list = tfidf_list # list

        # create target item sets
        self.sem_tag=Generate_tag(self.args.data_dir,self.args.data_name,self.args.data_dir)
        self.train_tag=self.sem_tag.get_data(self.args.data_dir+"/"+self.args.data_name+"_1_t.pkl","train") # 按照item分类，以某个item为target的一些用户序列在一个list中
        self.true_user_id,_,_,_,_=get_user_seqs(args.train_data_file) # user_id的list

    def _data_sample_rec_task(self, topk_ids, user_id, items, input_ids, target_pos, answer):
        ...
        if type(target_pos)==tuple:
            pad_len_1=self.max_len-len(target_pos[1]) # 和当前序列[-3] target相同的其他用户序列
    ...
    def __getitem__(self, index):
        ...
        items = self.user_seq[index] # 完整用户序列，不包含用户id，没有切割

        # print("index:", index)
        # print("tfidf_list:", len(self.tfidf_list))
        # print("user_id:", len(self.user_id))
        user_intent_dict = self.tfidf_list[self.user_id[index]-1] # self.user_id中的用户id从1开始
        topk_ids = heapq.nlargest(self.args.topk, user_intent_dict, key=user_intent_dict.get)
        #print("user_id:", type(user_id), user_id) # int,单个用户id，保存了256个用户的id(从1开始)
      

        assert self.data_type in {"train", "valid", "test"} # data_type只能在三者里面选
        ...
        if self.data_type == "train":
            ...
            temp=self.train_tag[items[-3]] # 找到和当前序列的target item相同的其他序列
            flag=False
            for t_ in temp:
                if t_[1:]==items[:-3]: # 如果只有一个item，pass
                    continue
                else:
                    target_pos_=t_[1:]
                    flag=True
        ...
        if self.data_type == "train":
            ...
            cur_rec_tensors = self._data_sample_rec_task(topk_ids, user_id, items, input_ids, target_pos, answer) # items只是序列，没有用户id
            return (cur_rec_tensors)
        elif self.data_type == "valid":
            cur_rec_tensors = self._data_sample_rec_task(topk_ids, user_id, items, input_ids, target_pos, answer)
            return cur_rec_tensors
        else:
            cur_rec_tensors = self._data_sample_rec_task(topk_ids, user_id, items_with_noise, input_ids, target_pos, answer)
            return cur_rec_tensors
...
def DS2(i_file, o_file, max_len):
    """
    :param i_file: input file path
    :param o_file: output file path
    :param max_len: max allowed length of a sequence (after user ID)
    """
    with open(i_file, "r+") as fr:
        data = fr.readlines()
    ...
    for d_ in data:
        parts = d_.strip().split()
        user_id = parts[0]
        items = parts[1:]

        aug_d.setdefault(user_id, [])
        seq_len = len(items)

        for start in range(seq_len):
            # 最少要有4个item
            for end in range(start + 4, min(seq_len + 1, start + max_len + 1)):
                subseq = items[start:end]
                aug_d[user_id].append(subseq)

    with open(o_file, "w+") as fw:
        for user_id in aug_d:
            for item_seq in aug_d[user_id]:
                fw.write(user_id + " " + ' '.join(item_seq) + "\n")


def DS3(i_file,o_file,max_len):
    """
    :param i_file: original data
    :param o_file: output data
    :max_len: the max length of the sequence
