...
from test import load_sae, select_intent, select_intent_random_no
'''
随机选择一些latent
'''
...
def main():
    ...
    #indices, latent_init_tensor =  select_intent_random_no(args.intent_file_path, sae_w) # 1208 torch.Size([1208, 768]) cuda:2
    latent_init_tensor = nn.Embedding(1208, 768)
        
    if args.encoder=="SAS":
        model = SASRecModel(args=args, latent_init_tensor=latent_init_tensor) # latent_init_tensor (1000,768)
