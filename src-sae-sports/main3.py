...
from datasets import RecWithContrastiveLearningDataset,DS, DS2

from trainers import ICSRecTrainer3
from models import SASRecModel,GRUEncoder, SASRecModel3
from utils import EarlyStopping, get_user_seqs, check_path, set_seed
from test import load_sae, select_intent
# match LateFusionEncoder3, SASRecModel3
# 简单串联，SASRec表示学完后接一个crossattention
...
def main():
    ...
    parser.add_argument("--output_dir", default="../output_sae/", type=str)
    ...
    parser.add_argument("--sae_file_path", type=str, default="../trained_sae/sae_model/TopK15_l8_epoch50_encoder_16384_beauty.pth", help="trained sae path")
    parser.add_argument("--intent_file_path", type=str, default="../trained_sae/intent_select/tunelens_beauty_TopK15_l8_epoch50_encoder_16384_keep15_mistral_prompt3_5000_intent.txt", help="trained sae path")
    ...
    # training data
    _,train_user_seq, _, train_valid_rating_matrix, train_test_rating_matrix = get_user_seqs(args.train_data_file) # list套list，一条序列
    # valid and test data
    '''我们不用分割数据的话，实际user_seq就是我们的train_user_seq'''
    _,user_seq, max_item, valid_rating_matrix, test_rating_matrix = get_user_seqs(args.data_file) # 没有分割过的数据，user_seq小于train_user_seq，没有分割
    ...
    train_dataset = RecWithContrastiveLearningDataset(args, train_user_seq, data_type="train") # 来自未分割的数据集
    train_sampler = RandomSampler(train_dataset) # 每次随机打乱顺序
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.batch_size)

    eval_dataset = RecWithContrastiveLearningDataset(args, user_seq, data_type="valid") # 来自未分割的数据集
    eval_sampler = SequentialSampler(eval_dataset) 
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size)

    test_dataset = RecWithContrastiveLearningDataset(args, user_seq, data_type="test") # 来自未分割的数据集
    ...
    if args.encoder=="SAS":
        model = SASRecModel3(args=args, latent_init_tensor=latent_init_tensor) # latent_init_tensor (1000,768)
    elif args.encoder=="GRU":
        model=GRUEncoder(args=args)
    trainer = ICSRecTrainer3(model, train_dataloader,cluster_dataloader, eval_dataloader, test_dataloader, args)
