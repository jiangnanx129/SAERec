...
from models import SASRecModel,GRUEncoder, SASRecModel_nogating
from utils import EarlyStopping, get_user_seqs, check_path, set_seed
from test import load_sae, select_intent, select_intent_random
'''
model用的不带gating的crossattention
'''
...
def main():
    ...
    # training data
    _,train_user_seq, _, _, _ = get_user_seqs(args.train_data_file) # list套list，一条序列
    # valid and test data
    ...
    with open(args.log_file, "a") as f:
        f.write(str(args) + "\n") # 参数写入log文件
        f.write("分割的数据, 不可训练sae, 两边并行后add, 有layernorm, 用的没有gating的crossattention, random的latent" + "\n") # 参数写入log文件
    ...
    indices, latent_init_tensor = select_intent_random(args.intent_file_path, sae_w) # 1208 torch.Size([1208, 768]) cuda:2

    if args.encoder=="SAS":
        model = SASRecModel_nogating(args=args, latent_init_tensor=latent_init_tensor) # latent_init_tensor (1000,768)
