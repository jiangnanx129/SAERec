...
class SASRecModel_ema(nn.Module):
    ...
    def forward(self, input_ids, topk_ids): # (b,l), (b,)
        ...
        # self, hidden_states, attention_mask, output_all_encoded_layers=True
        topk_indices, global_indices, item_encoded_layers, memory_tensor, query_repr, attention_scores_mean = self.item_encoder(sequence_emb, extended_attention_mask, topk_ids, self.indices, self.ema_tracker, output_all_encoded_layers=True)

        # sequence_output = item_encoded_layers[-1] 
        sequence_output = item_encoded_layers # 没有list保存两次的结果

        return topk_indices, global_indices, sequence_output, memory_tensor, query_repr, attention_scores_mean
